...
페인트를 칠해도 되지만 다시 칠하기로 정한 구역은 적어도 한 번 페인트칠을 
해야 합니다. 예산을 아끼기 위해 다시 칠할 구역을 정했듯 마찬가지로 롤러로 
페인트칠을 하는 횟수를 최소화하려고 합니다.


정수 n, m과 다시 페인트를 칠하기로 정한 구역들의 번호가 담긴 정수 배열 
section이 매개변수로 주어질 때 롤러로 페인트칠해야 하는 최소 횟수를 return 하는 
...
function solution(n, m, section) {
  ...
  let current = 0; // 마지막으로 페인트칠한 구역의 끝 위치

  for (let start of section) {
    // 현재 구역이 이전에 덮은 범위를 넘어섰다면 롤러 사용
    if (start > current) {
      count++; // 롤러 사용 횟수 증가
      current = start + m - 1; // 롤러로 덮은 마지막 구역 업데이트
    }
  }
