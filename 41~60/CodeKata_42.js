...
를 들어, k = 3, m = 4, 사과 7개의 점수가 [1, 2, 3, 1, 2, 3, 1]이라면,
...
function solution(k, m, score) {
  ...
  // k보다 큰 점수는 제외
  const validScores = sortedScore.filter(s => s <= k);

  // 내림차순한 배열을 쪼개자 
  let chunks = [];
    for (let i = 0; i < validScores.length; i += m) {
        chunks.push(validScores.slice(i, i + m));
    }  
