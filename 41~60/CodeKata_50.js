...
function solution(n, lost, reserve) {
  ...
  // 체육복 빌려주기
  filteredLost = filteredLost.filter(l => {
    const giverIndex = filteredReserve.findIndex(r => Math.abs(r - l) === 1);
    if (giverIndex !== -1) {
      filteredReserve.splice(giverIndex, 1); // 빌려준 학생 제거
      return false; // 체육복을 빌렸으므로 제거
    }
    return true; // 체육복을 못 빌리면 유지
  });
