...
function solution(sizes) {
  // 맵의 특성 배열을 복사한다.
  // 가로 세로 위치는 바뀔일이 없다고 했으니 스위칭하자.
  const temp = sizes.map(([w, h], index) => {
    return w > h ? [w, h] : [h, w]; // 큰 값을 앞에 배치
  });

  let w = Math.max(...temp.map(([w]) => w));
  let h = Math.max(...temp.map(([, h]) => h));

  // 위치는 바뀌지 않는다니 맘놓고 리턴하자.
  return w * h;
}

// 두번쨰 답안 (생각해보니깐 반복문 한번만 돌려도됬었음...)
function solution2(sizes) {
  let w = 0;
  let h = 0;

  // 하나의 반복문에서 스위칭하고, 최대값을 계산
  for (let [width, height] of sizes) {
    const [newWidth, newHeight] =
      width > height ? [width, height] : [height, width];
    w = Math.max(w, newWidth);
    h = Math.max(h, newHeight);
  }
